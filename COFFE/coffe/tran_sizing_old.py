...
def get_eval_area(fpga_inst, opt_type, subcircuit, is_ram_component, is_cc_component, is_dummy_component=0):
	...
	elif is_dummy_component == 1:
		return fpga_inst.area_dict['areaoverhead_dummy_total']
	else:
		return fpga_inst.area_dict["ram_core"]
		
	
def get_final_area(fpga_inst, opt_type, subcircuit, is_ram_component, is_cc_component, is_dummy_component=0):
	# Get area based on optimization type (subcircuit if local optimization, tile if global)
	if opt_type == "local":
		area = fpga_inst.area_dict[subcircuit.name]
		return area
...
def get_eval_delay(fpga_inst, opt_type, subcircuit, tfall, trise, low_voltage, is_ram_component, is_cc_component, is_dummy_component=0):
	...
	if opt_type == "local":
		...
		if is_dummy_component == 1:
			dummy_delay = fpga_inst.RAM.precharge_dummy.delay + fpga_inst.RAM.wordlinedriver_dummy.delay + fpga_inst.RAM.samp_dummy.delay + \
											fpga_inst.RAM.lookahead4_dummy.delay + fpga_inst.RAM.mux2_dummy.delay + fpga_inst.RAM.mux3_dummy.delay + fpga_inst.RAM.writedriver_dummy.delay
			return dummy_delay
...
def get_current_delay(fpga_inst, is_ram_component, is_dummy_component=0):
...
def get_final_delay(fpga_inst, opt_type, subcircuit, tfall, trise, is_ram_component, is_cc_component, is_dummy_component=0):
	...
	if delay < 0 :
			print "ERROR: final delay is negative"
...
def search_ranges(sizing_ranges, fpga_inst, sizable_circuit, opt_type, re_erf, area_opt_weight, 
				  delay_opt_weight, outer_iter, inner_iter, bunch_num, spice_interface, is_ram_component, is_cc_component, is_dummy_component=0):
	""" 
	...
	for combo in sizing_combos:
		...
		fpga_inst.update_area()
		# Get evaluation area
		area_list.append(get_eval_area(fpga_inst, opt_type, sizable_circuit, is_ram_component, is_cc_component, is_dummy_component))
		# Re-calculate wire lengths
		fpga_inst.update_wires()
		# Update wire resistance and capacitance
		fpga_inst.update_wire_rc()
	...
	for i in range(len(tfall_trise_list)):    
		...
		delay = get_eval_delay(fpga_inst, opt_type, sizable_circuit, tfall_trise[0], tfall_trise[1], meas_logic_low_voltage[i], is_ram_component, is_cc_component, is_dummy_component)
	...
	for i in range(re_erf_num):
		...
		delay = get_final_delay(fpga_inst, opt_type, sizable_circuit, tfall, trise, is_ram_component, is_cc_component, is_dummy_component)
		# Get area (run_combo will have updated the area numbers for us so we can get it 
		# directly)
		area = get_final_area(fpga_inst, opt_type, sizable_circuit, is_ram_component, is_cc_component, is_dummy_component)
		# Calculate cost
		cost = cost_function(area, delay, area_opt_weight, delay_opt_weight)
...
def size_fpga_transistors(fpga_inst, run_options, spice_interface):
	...
	while not is_done:
		...
		# To be removed later
		#######################################################
		## Size SRAM Dummy Array Wordline Driver
		#######################################################

		name = fpga_inst.RAM.wordlinedriver_dummy.name
		...
		if iteration == 1:
			...
			starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(fpga_inst.RAM.wordlinedriver_dummy.initial_transistor_sizes)
		else:
		...
		if quick_mode_dict[name] == 1:
			sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(fpga_inst, fpga_inst.RAM.wordlinedriver_dummy, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0, 1)
		else:
		...
		if quick_mode_dict[name] == 1:
			...
			current_cost =  cost_function(get_eval_area(fpga_inst, opt_type, fpga_inst.RAM.wordlinedriver_dummy, 1, 0, 1), fpga_inst.RAM.wordlinedriver_dummy.delay, area_opt_weight, delay_opt_weight)   
		...
		if fpga_inst.specs.enable_bram_block == 1:
			...
			if (fpga_inst.RAM.memory_technology =="SRAM") and (fpga_inst.specs.enable_cim == 1):
				...
				# Size the transistors of this subcircuit
				if quick_mode_dict[name] == 1:
					sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(fpga_inst, fpga_inst.RAM.lookahead4_dummy, "local", re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0, 1)
				else:
					sizing_results_dict[name]= sizing_results_list[len(sizing_results_list)-1][name]
					sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(sizing_results_list)-1][name]   
				...
				if quick_mode_dict[name] == 1:
					time_after_sizing = time.time()
					past_cost = current_cost
					current_cost =  cost_function(get_eval_area(fpga_inst, "local", fpga_inst.RAM.lookahead4_dummy, 1, 0, 1), fpga_inst.RAM.lookahead4_dummy.delay, area_opt_weight, delay_opt_weight)   
					if (past_cost - current_cost)/past_cost < fpga_inst.specs.quick_mode_threshold:
						quick_mode_dict[name] = 0

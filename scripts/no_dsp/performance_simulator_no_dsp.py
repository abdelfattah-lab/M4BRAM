...
def calc_m4bram_no_dsp_performance(network='alexnet', mode='s', variant='S', pw=8, pi=8,
                                rvec=3, dla_config={},
                                nn_dla=None, nn_dla_m4bram=None, 
                                num_dsp=1500, num_bram=1800,
                                num_bram_col=128, num_bram_row=128, bram_mux_factor=4, bram_dwidth=32,
    ...
    for pvec in [1, 2, 4]:
        for m4bram_qvec in [1, 2, 4]:
            if m4bram_qvec*pvec > 4:
                continue
            for m4bram_cvec in range(4, 129, 4):
                for m4bram_kvec in range(16, 257, 16):
                    if ((m4bram_kvec / m4bram_cvec) % 2) != 0:
                        continue
                    dla_m4bram = DLA_M4BRAM_no_DSP(network=network, mode=mode, variant=variant,
                                            pw=pw, pi=pi,
                    ...
                    if ((num_fcache_bram + num_streambuf_bram) > num_bram):
                        continue
                    ...
                    dla_m4bram_area     = calc_dla_m4bram_area(num_bram=num_fcache_bram + num_streambuf_bram, num_dsp=0, variant=variant)
                    ...
                    #print(m4bram_qvec, m4bram_cvec, m4bram_kvec, num_fcache_bram, speedup_total)

                    speedup_per_area = speedup_total / area_ratio
                    if speedup_total > 0.1:
                        speedup_per_area_list.append(speedup_per_area)
                        qf_list.append(speedup_total * speedup_per_area)
                    else:
                        speedup_per_area_list.append(0)
    ...
    df = pd.DataFrame(result_dict)
    df_result = df.sort_values(by='speedup_total', axis=0, ascending=False)

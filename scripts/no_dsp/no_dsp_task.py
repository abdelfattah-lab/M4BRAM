...
import re, os
import argparse
from performance_simulator_no_dsp import *
...
parser.add_argument('--weight_precision', type=int, required=True, default=8,
                    help='DNN weight precision \n')
parser.add_argument('--input_precision', type=int, required=True, default=8,
                    help='DNN Input precision \n')
...
def main():
    dla_config = calc_dla_mixed_act_performance(network=nn_name, rvec=rvec, pw=weight_precision, pi=input_precision, 
    ...
    df = calc_m4bram_no_dsp_performance(network=nn_name, mode=m4bram_mode, variant=m4bram_variant,
                                     rvec=rvec, pw=weight_precision, pi=input_precision, dla_config=dla_config, 
                                     num_dsp=num_dsp, num_bram=num_bram,
                                     num_bram_row=num_bram_row, num_bram_col=num_bram_col, 
                                     bram_mux_factor=bram_mux_factor, bram_dwidth=bram_dwidth, 
    ...
    with open(file_path, 'a') as f:
        f.write(str(mode+'-'+variant).ljust(15) + str(str(weight_precision)+'-bit').ljust(8) + \
                str(str(input_precision)+'-bit').ljust(8) + nn_name.ljust(10) + speedup.ljust(10) + '\n')

...
def calc_dla_mixed_weight_performance(network='alexnet', kvec_splitratio=0.9, 
                         pw_h=8, pw_l=4, pi=8, rvec=3, 
                         num_dsp=1500, num_bram=1800, 
                         nn_dla=None, ifmap_ofmap_size=None,
                         num_bram_row=128, num_bram_col=128, 
                         bram_mux_factor=4, bram_dwidth=32):
    if nn_dla == None or ifmap_ofmap_size == None:
        raise Exception('Please provide a correct neural network topology !')
    if (pi > 8) or (pi < 2) or (pw_h not in [2,4,8]) or (pw_l not in [2,4,8]):
        raise Exception('Invalid precision for weight or activation !')

    dla = DLA_mixed_weight(network=network, kvec_splitratio=kvec_splitratio, pw_h=pw_h, pw_l=pw_l, pi=pi, rvec=rvec, qvec=1, cvec=8, kvec=48)
    dla_latency = {}
    count = 1
    ...
    for pvec in pvec_searchspace:
        for qvec in qvec_searchspace:
            if (pw_h == 2) and (pvec*qvec)%2 != 0:
                continue
            for cvec in range(4, 65, 4):
                for kvec in range(4, 129, 4):
                    dlanew = DLA_mixed_weight(network=network, kvec_splitratio=kvec_splitratio, pw_h=pw_h, pw_l=pw_l, 
                                 pi=pi, rvec=rvec, pvec=pvec, qvec=qvec, cvec=cvec, kvec=kvec,
...
def calc_m4bram_mixed_weight_performance(network='alexnet', mode='s', variant='L', 
                                kvec_splitratio=0.9, pw_h=8, pw_l=4, pi=8,
                                rvec=3, dla_config={},
    ...
    dla = DLA_mixed_weight(network=network, kvec_splitratio=kvec_splitratio, 
              pw_h=pw_h, pw_l=pw_l, pi=pi, 
              rvec=rvec, pvec=dla_pvec, qvec=dla_qvec, cvec=dla_cvec, kvec=dla_kvec,
              num_bram_row=num_bram_row, num_bram_col=num_bram_col, bram_mux_factor=bram_mux_factor, bram_dwidth=bram_dwidth)
